...
class mysteryann(BaseOODANN):
    ...
    def fit(self, dataset):
        ...
        print('dataset metric:', str(ds.distance()))

        start = time.time()
        if ds.d == 200:
            m = self.translate_dist_fn(str(ds.distance()+'_build'))
        else:
            print('small dataset metric')
            save_path = ''
        ...
        if ds.d != 200:
            print('small dataset, no training queries')
    ...
    def query(self, X, k):
        ...
        self.index.search(X, k, self.L_pq, self.res, X.shape[0], self.search_threads)
    ...
    def __str__(self):
        return f'mysteryann({self.index_name(), self._query_args})'